...
//       id: 1,
//       sender: "ai",
//       text: "Hi, this is PackPalz 👋 Nice to meet you. Tell me your current mood.",
//     },
//   ]);
...
//     { place: string; activities: string[] }[]
//   >([]);
...
// const [extraNotes, setExtraNotes] = useState("");  const [travelMode, setTravelMode] = useState<"solo" | "friends" | "strangers" | "">("");
...
//     if (step === "askDays") {
//       setDays(value);
//       addMessage("ai", "If you have a specific date, please mention it. If not, type 'no'.");
//       setStep("askDate");
//       return;
//     }
...
//       addMessage(
//         "ai",
//         "Who do you want to travel with? Type one: solo, friends, or strangers."
//       );
//       setStep("askTravelMode");
//       return;
//     }

//     if (step === "askTravelMode") {
//       const normalized = value.toLowerCase();

//       if (
//         normalized !== "solo" &&
//         normalized !== "friends" &&
//         normalized !== "strangers"
//       ) {
//         addMessage("ai", "Please type one of these: solo, friends, strangers.");
//         return;
//       }

//       setTravelMode(normalized as "solo" | "friends" | "strangers");
...
//             <div className="floating-ai-itinerary-box">
//               {itineraryText}
//             </div>

//             <p className="floating-ai-confirm-text">
//               Is this itinerary okay to you?
//             </p>
...
//               placeholder={
//                 loading ? "Please wait..." : "Type your reply here..."
//               }
//               disabled={loading}
//             />
...
export default function FloatingAIChat({
  ...
  const [activitiesByPlace, setActivitiesByPlace] = useState<ActivityGroup[]>([]);
  const [selectedActivities, setSelectedActivities] = useState<SelectedActivitiesGroup[]>([]);
  ...
  const handleGetActivities = async () => {
    try {
      ...
      addMessage(
        "ai",
        res.data.message || "Nice choices. Now select the activities you would like to do."
      );
      setStep("selectActivities");
    } catch (error) {
    ...
    <div className={`floating-ai-chat ${fullPage ? "full-page" : ""}`}>
      <div className="floating-ai-messages">
        {messages.map((message) => (
          <div
            key={message.id}
            className={`floating-ai-message-row ${message.sender === "user" ? "user" : "ai"}`}
          >
            <div className={`floating-ai-message-bubble ${message.sender}`}>
              <p>{message.text}</p>
            </div>
